...
namespace stdext
{
    ...
    template <typename Iterator> struct is_iterator
        : std::conditional_t<_private::STDEXT_HAS_INNER_TYPE_V(std::iterator_traits<Iterator>, iterator_category),
            true_type,
            false_type>
    { };

    template <typename Iterator> using iterator_category = typename std::iterator_traits<Iterator>::iterator_category;
    namespace _private
    {
